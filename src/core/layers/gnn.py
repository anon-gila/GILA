...
class GAT(nn.Module):
    def __init__(self, nfeat, nhid, nclass,fake_idx, dropout, alpha, nheads):
        ...
        self.one_prompt = nn.Parameter(torch.Tensor(1, nfeat))
        glorot(self.one_prompt) ## init    
        if fake_idx is not None:
            self.prompts = nn.Parameter(torch.Tensor(fake_idx.shape[0], nfeat)) 
            glorot(self.one_prompt) ## init
            glorot(self.prompts) ## init
            

     
    def add_prompt(self,x, zero_matrix): 
        #### 2d smote prompt
        prompt = torch.concat((zero_matrix, self.prompts), dim=0)
        x = x+prompt
        return x
    
    def add_one_prompt(self, x):
        #### 1d smote prompt
        prompt = torch.zeros_like(x)
        prompt[self.fake_idx]= 1
        x = x + (prompt * self.one_prompt)
        # x[self.fake_idx] += self.one_prompt
        # print(x)
        return x
    
    def add_all_prompt(self,x):
        #### 1d all node prompt
        return x + self.one_prompt
...
class GCN(nn.Module):
    def __init__(self, nfeat, nhid, nclass,fake_idx, graph_hops, dropout, batch_norm=False):
        ...
        if fake_idx is not None:
            self.updaters = nn.Parameter(torch.Tensor(fake_idx.shape[0], nfeat)) 
            glorot(self.updaters) ## init
            

     
    def update_zero_node(self,x, zero_matrix): 
        prompt = torch.concat((zero_matrix, self.updaters), dim=0)
        x = x+prompt
        return x
