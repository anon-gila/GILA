...
def prepare_datasets(config, args, data_index):
    ...
    if config['data_type'] == 'network':
        ...
        # data_conf = uci_data_utils.UCI(seed=args['data_seed'], dataset_name=config['dataset_name'], n_train=config['n_train'], n_val=config['n_val'], ratio=args['ratio'],fake_samples=args['fake_samples'],fake_rate=args['fake_rate'], smote=args['smote'],whole_minor=args['whole_minor'], noise_rate=args['noise_rate'])
        data_conf = uci_data_utils.UCI(seed=args['data_seed'], dataset_name=config['dataset_name'], n_train=config['n_train'], n_val=config['n_val'], ratio=args['ratio'],fake_samples=args['fake_samples'],fake_rate=args['fake_rate'], smote=args['smote'])
        
        if args['smote']==True or args['fake_samples']==True:
    
            adj, features, labels, idx_train, idx_val, idx_test, fake_idx = data_conf.load(data_index = data_index,data_dir=config.get('data_dir', None), knn_size=args['knn_size'], epsilon=config.get('input_graph_epsilon', None), knn_metric=config.get('knn_metric', 'cosine'), ratio = args['ratio'])
            device = config['device']
            data = {'adj': adj.to(device) if device and adj is not None else adj,
                    ...
                    'idx_val': idx_val.to(device) if device else idx_val,
                    'idx_test': idx_test.to(device) if device else idx_test,
                    'fake_idx': fake_idx.to(device) if device else fake_idx}
        else:
            # adj, features, labels, idx_train, idx_val, idx_test = data_conf.load(data_index = data_index,data_dir=config.get('data_dir', None), knn_size=config['input_graph_knn_size'], epsilon=config.get('input_graph_epsilon', None), knn_metric=config.get('knn_metric', 'cosine'), ratio = args['ratio'])
            adj, features, labels, idx_train, idx_val, idx_test = data_conf.load(data_index = data_index,data_dir=config.get('data_dir', None), knn_size=args['knn_size'], epsilon=config.get('input_graph_epsilon', None), knn_metric=config.get('knn_metric', 'cosine'), ratio = args['ratio'])
